...
$totalRows = 0;
$tin = $t->TinTrongLoai($idLT, $pageNum, $pageSize, $totalRows); //chỉ lấy 1 trang thứ $pageNum vs $pageSize record
...
<div id="tintrongloai">
    ...
    <?php while ($row_tin = $tin->fetch_assoc()) { ?>
        <div class="stuff_article_inner"> <span class="stuff_date"><?=date('M', strtotime($row_tin['Ngay'])) ?> <strong><?=date('d', strtotime($row_tin['Ngay'])) ?></strong></span>
            ...
            <h2><a href="index.php?p=chitiettin&idTin=<?=$row_tin['idTin']; ?>"><?=$row_tin['TieuDe'] ?></a></h2>
            <p><?=$row_tin['TomTat'] ?></p>
        </div>
    <?php } ?>
</div>
...
<div class="slideInLeft animated">
    <?=$t->pageList("index.php?p=tintrongloai&idLT=$idLT", $totalRows, $pageNum, $pageSize, $offset=3); ?>
</div>
